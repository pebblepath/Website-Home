...
`)}class Se extends D{constructor(){super(),this.trip=null,this.members=[],this._resizeObs=null}connectedCallback(){super.connectedCallback(),typeof ResizeObserver<"u"&&(this._resizeObs=new ResizeObserver(()=>this._fitTitle()))}disconnectedCallback(){var e;super.disconnectedCallback(),(e=this._resizeObs)==null||e.disconnect()}updated(){if(this._resizeObs&&this.renderRoot){const e=this.renderRoot.querySelector("article");e&&!e._observed&&(this._resizeObs.observe(e),e._observed=!0)}this._fitTitle()}_fitTitle(){if(!this.renderRoot)return;const e=this.renderRoot.querySelector("h3");if(!e)return;e.style.fontSize="";let t=19;for(e.style.fontSize=`${t}px`;e.scrollWidth>e.clientWidth+1&&t>13;)t-=.5,e.style.fontSize=`${t}px`}_fmtDates(e,t){const i=C(e),r=C(t);if(!i||!r)return"";const a=i.toLocaleString("en-GB",{month:"short"}),n=r.toLocaleString("en-GB",{month:"short"});return a===n&&i.getFullYear()===r.getFullYear()?`${i.getDate()} – ${r.getDate()} ${a}`:`${i.getDate()} ${a} – ${r.getDate()} ${n}`}async _onShare(e,t,i){i.stopPropagation();const r=Yt(e,t);if(navigator.share)try{await navigator.share({title:`Cairn — ${e.title??"activity"}`,text:r})}catch{}else try{await navigator.clipboard.writeText(r),x("Itinerary copied to clipboard.")}catch{x("Could not copy — try again from a browser tab.")}}render(){const e=this.trip;if(!e)return s``;const t=e.coverImage?`background-image: url(${e.coverImage});`:`background: ${Rt(e)};`,i=e.coverImage?"cover has-image":"cover",r=new Map(this.members.map(l=>[l.uid,l])),a=(e.attendees??[]).map(l=>r.get(l)).filter(Boolean),n=a.slice(0,4),o=Math.max(0,a.length-n.length);return s`
  ...
  `);customElements.define("profile-sheet",Ne);const ie=class ie extends D{constructor(){super(),this.open=!1}static get OPTIONS(){return[{type:"activity",tone:"sage",icon:s`<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
    ...
    `:s``}};f(ie,"properties",{open:{type:Boolean,reflect:!0}}),f(ie,"styles",E`
    * { box-sizing: border-box; }
    :host {
      position: fixed;
      inset: 0;
  ...
  `);customElements.define("join-family-screen",Me);class Oe extends D{constructor(){super(),this.error="",this.busy=!1,this.joinCode="",this._codeInputOpen=!1,this._code=""}async _handleSignIn(){if(this.busy)return;const e=(this._code??"").trim().toUpperCase();if(this._codeInputOpen&&e){const t=e.startsWith("CAIRN-")?e:`CAIRN-${e.replace(/^CAIRN-?/i,"")}`;try{localStorage.setItem("cairn:pendingJoinCode",t)}catch{}}this.busy=!0,this.error="";try{await ee()}catch(t){this.error=(t==null?void 0:t.message)??"Sign-in failed."}finally{this.busy=!1}}_toggleCode(){this._codeInputOpen=!this._codeInputOpen,this._codeInputOpen&&requestAnimationFrame(()=>{var e;(e=this.renderRoot.querySelector(".code-input"))==null||e.focus()})}_renderGoogleIcon(){return s`
  ...
  `);customElements.define("register-screen",je);const V=class V extends D{constructor(){super(),this.value="family"}_select(e){e!==this.value&&(this.value=e,this.dispatchEvent(new CustomEvent("circle-change",{detail:{value:e},bubbles:!0,composed:!0})))}render(){return s`
    ...
    `}};f(V,"properties",{value:{type:String,reflect:!0}}),f(V,"OPTIONS",[{value:"personal",label:"Just me"},{value:"family",label:"Family"},{value:"extended",label:"Extended"}]),f(V,"styles",E`
    * {
      box-sizing: border-box;
    }
    :host {
  ...
  `);let Le=V;customElements.define("circle-switcher",Le);class Ge extends D{constructor(){super(),this.event=null,this.members=[]}_icon(e){return e==="birthday"?s`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
        ...
        <section>
          <div class="cel-cairn-row">
            ...
            <div class="cel-cairn-col">
              ...
              ${(()=>{const y=(M,H)=>String(M.date).localeCompare(String(H.date)),I=t.filter(M=>M.type==="birthday").slice().sort(y),R=t.filter(M=>M.type==="anniversary").slice().sort(y),Z=t.filter(M=>M.type!=="birthday"&&M.type!=="anniversary").slice().sort(y),Q=(M,H,re)=>s`
            ...
            <div class="cel-cairn-col">
              ...
              <glass-panel padding="md" variant="strong">
                ${(()=>{var re,ae,se,Ye,He;const y=i.find(L=>{var B;return L.uid===((B=this.user)==null?void 0:B.uid)}),R=new Set(((re=this.family)==null?void 0:re.memberIds)??[]).has((ae=this.user)==null?void 0:ae.uid),Z=i.filter(L=>{var B;return L.uid!==((B=this.user)==null?void 0:B.uid)}),Q=!R&&y?[{...y,role:"extended"},...r]:r,M=Object.entries(((se=this.family)==null?void 0:se.subGroups)??{}),H=new Map(a.map(L=>[L.uid,L]));return s`
...
//# sourceMappingURL=index-B_mc-3LQ.js.map
