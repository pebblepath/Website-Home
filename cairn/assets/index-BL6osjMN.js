...
`)}class ke extends z{constructor(){super(),this.trip=null,this.members=[],this._resizeObs=null}connectedCallback(){super.connectedCallback(),typeof ResizeObserver<"u"&&(this._resizeObs=new ResizeObserver(()=>this._fitTitle()))}disconnectedCallback(){var e;super.disconnectedCallback(),(e=this._resizeObs)==null||e.disconnect()}updated(){if(this._resizeObs&&this.renderRoot){const e=this.renderRoot.querySelector("article");e&&!e._observed&&(this._resizeObs.observe(e),e._observed=!0)}this._fitTitle()}_fitTitle(){if(!this.renderRoot)return;const e=this.renderRoot.querySelector("h3");if(!e)return;e.style.fontSize="";let t=19;for(e.style.fontSize=`${t}px`;e.scrollWidth>e.clientWidth+1&&t>13;)t-=.5,e.style.fontSize=`${t}px`}_fmtDates(e,t){const i=D(e),r=D(t);if(!i||!r)return"";const a=i.toLocaleString("en-GB",{month:"short"}),n=r.toLocaleString("en-GB",{month:"short"});return a===n&&i.getFullYear()===r.getFullYear()?`${i.getDate()} – ${r.getDate()} ${a}`:`${i.getDate()} ${a} – ${r.getDate()} ${n}`}async _onShare(e,t,i){i.stopPropagation();const r=St(e,t);if(navigator.share)try{await navigator.share({title:`Cairn — ${e.title??"activity"}`,text:r})}catch{}else try{await navigator.clipboard.writeText(r),x("Itinerary copied to clipboard.")}catch{x("Could not copy — try again from a browser tab.")}}render(){const e=this.trip;if(!e)return o``;const t=e.coverImage?`background-image: url(${e.coverImage});`:`background: ${Ct(e)};`,i=e.coverImage?"cover has-image":"cover",r=new Map(this.members.map(l=>[l.uid,l])),a=(e.attendees??[]).map(l=>r.get(l)).filter(Boolean),n=a.slice(0,4),s=Math.max(0,a.length-n.length);return o`
  ...
  `);let Ee=X;customElements.define("circle-switcher",Ee);class Ie extends z{constructor(){super(),this.event=null,this.members=[]}_icon(e){return e==="birthday"?o`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
    ...
    `}_openCreate(){if(this.preview){x("Sign in to create real activities.");return}if(!_.familyId){x("You need a PebblePath family first.");return}this._typePickerOpen=!0}_onTypePicked(e){this._typePickerOpen=!1;const t=e.detail.type;if(t==="event"){this._eventFormEvent=null,this._eventFormOpen=!0;return}if(t==="import"){this._importOpen=!0;return}this._formMode=t,this._formTrip=null,this._formOpen=!0}_openEdit(e){if(this.preview){x("Sign in to edit real activities.");return}const t=e.lodgingUrl||e.lodgingHost||e.flightNumber||e.flightDepartAirport;this._formMode=t?"trip":"activity",this._formTrip=e,this._formOpen=!0}async _onSaveTrip(e){const t=e.detail;this._formBusy=!0;try{await _.saveTrip(t),this._formOpen=!1,this._formTrip=null,x(t.id?"Trip updated.":"Trip created.")}catch(i){console.error("Save trip failed:",i),x(`Couldn't save: ${i.code??i.message}`,{duration:5e3})}finally{this._formBusy=!1}}async _onDeleteTrip(e){this._formBusy=!0;try{await _.deleteTrip(e.detail.id),this._formOpen=!1,this._formTrip=null,x("Trip deleted.")}catch(t){console.error("Delete trip failed:",t),x(`Couldn't delete: ${t.code??t.message}`,{duration:5e3})}finally{this._formBusy=!1}}_openCreateEvent(){if(this.preview){x("Sign in to add real events.");return}if(!_.familyId){x("You need a family first.");return}this._eventFormEvent=null,this._eventFormOpen=!0}_openEditEvent(e){if(this.preview){x("Sign in to edit real events.");return}if(e!=null&&e._childId){const t=prompt(`Edit ${e._childName}'s birthday (YYYY-MM-DD):`,e.date);if(!t)return;if(!/^\d{4}-\d{2}-\d{2}$/.test(t)){x("Use YYYY-MM-DD format.");return}_.updateChildBirthday(e._childId,new Date(t)).then(()=>x(`Updated ${e._childName}'s birthday.`)).catch(i=>{console.error("Update child birthday failed:",i),x(`Couldn't update: ${i.code??i.message}`,{duration:5e3})});return}this._eventFormEvent={...e,date:e._originalDate??e.date},this._eventFormOpen=!0}async _onSaveEvent(e){this._eventFormBusy=!0;try{await _.saveEvent(e.detail),this._eventFormOpen=!1,this._eventFormEvent=null,x(e.detail.id?"Event updated.":"Event added.")}catch(t){console.error("Save event failed:",t),x(`Couldn't save: ${t.code??t.message}`,{duration:5e3})}finally{this._eventFormBusy=!1}}async _onDeleteEvent(e){this._eventFormBusy=!0;try{await _.deleteEvent(e.detail.id),this._eventFormOpen=!1,this._eventFormEvent=null,x("Event deleted.")}catch(t){console.error("Delete event failed:",t),x(`Couldn't delete: ${t.code??t.message}`,{duration:5e3})}finally{this._eventFormBusy=!1}}render(){var w,f,g,m,k,A,T,E,P,O,W;const e=this._filteredTrips(),t=this._filteredEvents(),i=this._liveImmediate(),r=this._liveExtended(),a=i.concat(r),n=(((w=this.user)==null?void 0:w.displayName)??"there").split(" ")[0],s=new Date,l=new Date(s.getFullYear(),s.getMonth(),1),c=new Date(s.getFullYear(),s.getMonth()+1,0),h=t.filter(v=>{const N=D(v.date);return N&&N.getFullYear()===s.getFullYear()&&N.getMonth()===s.getMonth()}),u=this._circleTrips().filter(v=>{if(!v.start||!v.end)return!1;const N=D(v.start),R=D(v.end);return Number.isNaN(N.getTime())||Number.isNaN(R.getTime())?!1:N<=c&&R>=l}).length+h.length;return o`
      ...
      <main>
        ...
        <section>
          ...
          ${(()=>{const v=(I,B)=>String(I.date).localeCompare(String(B.date)),N=t.filter(I=>I.type==="birthday").slice().sort(v),R=t.filter(I=>I.type==="anniversary").slice().sort(v),V=t.filter(I=>I.type!=="birthday"&&I.type!=="anniversary").slice().sort(v),Q=(I,B,ee)=>o`
...
//# sourceMappingURL=index-BL6osjMN.js.map
