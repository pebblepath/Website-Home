...
class FamilyDataStore extends EventTarget {
  constructor() {
    ...
    this.state = {
      ...
      // Non-parent Family Brief (2026-06-17) — the child-dev-free brief
      // shown on a NON-PARENT member's Today tab. Subscribed on the
      // Cairn-resolved family (runs for everyone; only non-parents render
      // it). Generated by the generateNonParentBrief CF / the nightly
      // scheduledFamilyBriefs precompute.
    ...
    // Non-parent Family Brief (2026-06-17) — also on the Cairn-resolved
    // family (so a non-parent with no own household still receives it).
    this._unsubNonParentDaily = null;
  ...
  start(uid) {
    ...
    this._unsubUser = onSnapshot(doc(db, 'users', uid), (snap) => {
      ...
      if (fid !== this._currentFamilyId) {
        ...
        this.state.nonParentDailyCard = null;
  ...
  _subscribeFamily(familyId) {
    ...
    // Non-parent Family Brief (2026-06-17) — the child-dev-free brief for a
    // NON-PARENT member's Today tab. Lives at
    // /families/{fid}/nonParentDailyCards/{YYYY-MM-DD}; readable by the
    // whole circle (rule: isCairnMember), written admin-only by the
    // generateNonParentBrief CF. Subscribed HERE (on the Cairn-resolved
    // family, which runs for everyone) rather than in _subscribePpFamily,
    // because a non-parent has NO own PP household and never hits that
    // path. Today-only via generatedAt (mirrors the familyDailyCards
    // listener). Only the non-parent Today branch renders it; parents get
    // an idle subscription. No content predicate on the rule, so the
    // doc-collection query is admitted + sorted client-side (no index).
    this._unsubNonParentDaily?.();
    const cachedNp = this._readBriefCache('nonparent', familyId);
    if (cachedNp) this.state.nonParentDailyCard = cachedNp;
    this._unsubNonParentDaily = onSnapshot(
      collection(db, 'families', familyId, 'nonParentDailyCards'),
      (snap) => {
        ...
        this.state.nonParentDailyCard = freshToday ? latest : null;
        if (freshToday) this._writeBriefCache('nonparent', familyId, latest);
        this._emit();
      },
      (err) =>
        console.warn('[Portal] nonParentDailyCards error:', err.code, err.message),
    );
  }
  ...
  /**
   * Non-parent Family Brief (2026-06-17) — build + write today's
   * child-dev-free brief SERVER-SIDE via the `generateNonParentBrief`
   * callable, on the CAIRN-resolved family (`_currentFamilyId`, which a
   * non-parent has — they have no own PP household). The CF is the only
   * writer (admin-only rule); the live `_unsubNonParentDaily` listener
   * picks up the overwrite. Returns the card, or null when there's
   * nothing scheduled to brief (the CF reports failed-precondition) so
   * the caller shows the calm placeholder instead of an error.
   */
  async refreshNonParentBrief() {
    if (!functions) throw new Error('Firebase functions not configured.');
    if (!this._currentFamilyId) throw new Error('No family.');
    const fn = httpsCallable(functions, 'generateNonParentBrief');
    try {
      const result = await fn({ familyId: this._currentFamilyId });
      return result.data;
    } catch (e) {
      // failed-precondition = "nothing to brief today" → not an error.
      if (e?.code === 'functions/failed-precondition') return null;
      throw e;
    }
  }
